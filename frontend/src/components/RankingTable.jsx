        ...
        <table className="w-full min-w-max divide-y divide-line/80 text-sm">
          <thead className="bg-canvas/65">
            <tr className="text-left text-xs uppercase tracking-[0.22em] text-muted">
              <th className="whitespace-nowrap px-4 py-4 font-medium">Pos.</th>
              ...
              <th className="whitespace-nowrap px-4 py-4 font-medium">Exatos</th>
              <th className="whitespace-nowrap px-4 py-4 font-medium">Empates</th>
              <th className="whitespace-nowrap px-4 py-4 font-medium">Vencedores</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-line/60">
            {ranking.map((entry) => (
              <tr key={entry.user_id} className="bg-panel/40 text-sm text-ink">
                <td className="whitespace-nowrap px-4 py-4 font-display text-lg font-semibold text-accent">
                  {entry.rank}
                </td>
                ...
                <td className="whitespace-nowrap px-4 py-4 text-lg font-semibold">{entry.total_points}</td>
                <td className="whitespace-nowrap px-4 py-4 text-muted">{entry.exact_hits}</td>
                <td className="whitespace-nowrap px-4 py-4 text-muted">{entry.draw_tendency_hits}</td>
                <td className="whitespace-nowrap px-4 py-4 text-muted">{entry.winner_tendency_hits}</td>
              </tr>
            ))}
            {ranking.length === 0 ? (
