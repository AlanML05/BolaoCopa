Create one horizontal animation strip for Codex pet `shellbyte`, state `running-right`.
...
Identity: same pet in every frame: A small friendly 3D-toy coding mascot named Shellbyte: deep PowerShell-blue body, teal accents, tiny database-cylinder backpack, subtle soccer-ball charm, small approval clipboard/checkmark prop, practical focused expression, production-ready but warm; no readable text or logos.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `3d-toy`: Stylized 3D toy mascot with smooth rounded forms, simple materials, clear silhouette, and no photoreal complexity. User style notes: polished compact toy mascot, readable at 192x208, not photorealistic, crisp silhouette, flat removable chroma-key background.
